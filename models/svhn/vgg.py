...
cfg = {
    ...
    'F' : [32, 32, 'M', 64, 64, 'M', 128, 128, 'M', 256, 256, 'M'],
}
...
def vgg11_bn():
    return VGG(make_layers(cfg['A'], batch_norm=True)[0])

def vgg13_bn():
    return VGG(make_layers(cfg['B'], batch_norm=True)[0])

def vgg16_bn():
    return VGG(make_layers(cfg['D'], batch_norm=True)[0])
